...
void InternalBuildChannels(int input) {
    ...
    for(int ii = 0; ii < input; ii++) {
        ...
        [_engine->mixer connect:channel->node to:_engine->mixer.mainMixerNode format:nil];
...
void InternalBuildBackground() {
    ...
    [_engine->mixer connect:player->node to:_engine->mixer.mainMixerNode format:nil];
